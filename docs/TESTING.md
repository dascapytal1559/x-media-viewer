...
4. Check E for sound and Space for playback. Confirm video playback starts and the poster disappears after a frame becomes available.
...
- Keep the first GitHub repository private. Select a license before making a public release; the current package is marked `UNLICENSED` and is not publishable to npm.
